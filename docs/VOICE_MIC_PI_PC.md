...
| `GLADOS_SD_OUTPUT_DEVICE` / `PI_SD_OUTPUT_DEVICE` | Pi | PortAudio **output** index for TTS. |
| `PI_AUDIO_OUTPUT_SR` | Pi | Force sample rate (e.g. `48000`) if probing fails. |
| `PI_AUDIO_OUTPUT_CHANNELS` | Pi | `1` or `2` to force; default probes **mono then stereo**. |
